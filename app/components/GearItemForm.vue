...
<template>
  <form class="flex flex-col gap-3 rounded border border-line p-4" @submit.prevent="submit">
    <p class="font-medium">{{ catalogItemLabel }}</p>

    <!-- Der Hinweis erscheint erst nach der Auswahl und ist nie ein Pflichtfeld. -->
    <p v-if="showPrecisionHint && !hintDismissed" class="rounded bg-surface-2 p-2 text-sm">
    ...
    <label class="flex flex-col gap-1">
      ...
      <input v-model="year" type="number" min="1900" max="2100" class="rounded border border-line px-3 py-2" />
    </label>
    <label class="flex flex-col gap-1">
      <span class="text-sm">{{ t.gearForm.finish }}</span>
      <input v-model="finish" type="text" maxlength="80" class="rounded border border-line px-3 py-2" />
    </label>
    <label class="flex flex-col gap-1">
      <span class="text-sm">{{ t.gearForm.modifications }}</span>
      <input v-model="modifications" type="text" maxlength="500" class="rounded border border-line px-3 py-2" />
    </label>
    <label class="flex flex-col gap-1">
      <span class="text-sm">{{ t.gearForm.notes }}</span>
      <textarea v-model="notes" maxlength="1000" rows="3" class="rounded border border-line px-3 py-2" />
    </label>
    <label v-if="ownedGear.length > 0" class="flex flex-col gap-1">
      <span class="text-sm">{{ t.gearForm.installedIn }}</span>
      <select v-model="installedInId" class="rounded border border-line px-3 py-2">
    ...
    <div class="flex gap-2">
      <button type="submit" class="rounded bg-accent px-4 py-2 text-accent-ink">{{ t.rig.save }}</button>
      <button type="button" class="rounded border border-line px-4 py-2" @click="emit('cancel')">{{ t.rig.cancel }}</button>
    </div>
  </form>
</template>
